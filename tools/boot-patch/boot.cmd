...
# XMIO recovery hook: set at the U-Boot prompt to force one-off values that
# survive this import (these names are intentionally NOT in armbianEnv.txt):
#   setenv xmio_fdt_override rk3128-linux.dtb; boot
#   setenv xmio_overrides "usb-otg-host uart1 dmc-disabled"; boot
if test -n "${xmio_fdt_override}"; then
    echo "[XMIO] fdtfile override: ${xmio_fdt_override}"
    setenv fdtfile "${xmio_fdt_override}"
fi
if test -n "${xmio_overrides}"; then
    echo "[XMIO] overlays override: ${xmio_overrides}"
    setenv overlays "${xmio_overrides}"
fi
