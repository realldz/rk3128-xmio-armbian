#!/usr/bin/env bash
# planb-v19.3.sh — DTB: drop heartbeat default-trigger (boot race made the
# dual LED look green; trigger also swallowed service brightness writes).
# zImage unchanged (v19). Flash BOTH boot.img and resource.img.
set -euo pipefail
...
echo "=== 1. patch dts ==="
python3 /workspace/tools/v19.3-dts.py "$W/xmio-planb-v19.2.dts" "$W/xmio-planb-v19.3.dts"

echo "=== 2. compile + verify ==="
dtc -I dts -O dtb -o "$OUT/rk3128-xmio-planb.dtb" "$W/xmio-planb-v19.3.dts" 2>/dev/null
dtc -I dtb -O dts "$OUT/rk3128-xmio-planb.dtb" 2>/dev/null > /tmp/v193-check.dts
python3 - <<'EOF'
d = open('/tmp/v193-check.dts').read()
...
import re
assert 'heartbeat' not in leds, 'heartbeat still present!'
assert re.search(r'gpios = <0x[0-9a-f]+ 0x08 0x00>', leds), 'dual changed!'
assert re.search(r'gpios = <0x[0-9a-f]+ 0x09 0x01>', leds), 'yellow not active-low!'
print('DTB verify OK (no heartbeat, yellow ACTIVE_LOW kept)')
EOF
