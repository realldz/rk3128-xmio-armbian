#!/usr/bin/env bash
# planb-v17.2.sh — fix ftlv5 usleep_range quantization (20ms/page -> µs).
# Expected effect: raw read/write throughput jumps from ~770 kB/s to the
# die's real capability (several MB/s on worn MLC). Flash: boot.img ->
# 0xE000, resource.img -> 0x6800. rootfs/cmdline unchanged.
set -euo pipefail
...
echo "=== 1. patch blob usleep -> udelay helper ==="
python3 /workspace/tools/v17.2-udelay.py

echo "=== 2. rebuild zImage ==="
cd "$SRC"
export CROSS_COMPILE="$CROSS" ARCH=arm LOCALVERSION=+
make -j"$(nproc)" O="$B" zImage > /workspace/work/v17.2-build.log 2>&1 || {
  tail -40 /workspace/work/v17.2-build.log; exit 1; }
tail -3 /workspace/work/v17.2-build.log

echo "=== 2b. verify: no usleep_range left in blob object, helper linked ==="
if strings "$B/vmlinux" | grep -q "FtlWrite: lpa error"; then echo "blob strings OK"; fi
nm "$B/drivers/rk_nand/rk_ftlv5_arm32.o" | grep -c usleep_range || true
nm "$B/vmlinux" | grep -w 'rk_ftl_udelay' || echo "MISSING rk_ftl_udelay"

echo "=== 3. build v17.2 DTB (v17 source, unchanged) ==="
...
dtc -I dtb -O dts "$OUT/rk3128-xmio-planb.dtb" 2>/dev/null > /tmp/v172-check.dts
python3 - <<'EOF'
d = open('/tmp/v172-check.dts').read()
...
assert '\ttimer@20044020 {' in d, 'timer2 lost!'
a = d.index('\ttimer@20044000 {')
b2 = d.index('\ttimer@20044020 {')
assert a < b2, 'timer order wrong!'
print('DTB verify: MAC+sdmmc+timer2 all present, order OK')
EOF
...
cd "$OUT" && rm -f SHA256SUMS.txt && sha256sum \
  ...
  parameter.txt.badnand1 uboot-stock.img misc.img baseparamer-720P.img \
