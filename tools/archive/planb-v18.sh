#!/usr/bin/env bash
# planb-v18.sh — HDMI poll fallback (EDID without HPD IRQ) + same v17.2 kernel fixes.
...
echo "=== 1. hdmi poll fallback ==="
python3 /workspace/tools/v18-hdmi-poll.py

echo "=== 2. rebuild zImage ==="
cd "$SRC"
export CROSS_COMPILE="$CROSS" ARCH=arm LOCALVERSION=+
make -j"$(nproc)" O="$B" zImage > /workspace/work/v18-build.log 2>&1 || {
  tail -40 /workspace/work/v18-build.log; exit 1; }
tail -3 /workspace/work/v18-build.log
...
dtc -I dtb -O dts "$OUT/rk3128-xmio-planb.dtb" 2>/dev/null > /tmp/v18-check.dts
python3 - <<'EOF'
d = open('/tmp/v18-check.dts').read()
