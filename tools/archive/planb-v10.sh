#!/usr/bin/env bash
# planb-v10.sh — Plan B v10: SIP stub + mach-rockchip enabled (SMP) on top of v9.
# Runs INSIDE docker rk3128-build:  bash /workspace/tools/planb-v10.sh
# Only boot.img changes (kernel). DTB/parameter/resource unchanged -> flash boot.img only.
set -euo pipefail
...
echo "=== 1. apply v10 patches (on top of v9) ==="
python3 /workspace/tools/v10-patch.py "$SRC" | tee /workspace/work/v10-patch.log

echo "=== 2. incremental zImage build ==="
cd "$SRC"
export CROSS_COMPILE="$CROSS" ARCH=arm LOCALVERSION=+
make -j"$(nproc)" O="$B" zImage > /workspace/work/v10-build.log 2>&1 || {
  tail -40 /workspace/work/v10-build.log; exit 1; }
tail -4 /workspace/work/v10-build.log

echo "=== 2b. verify mach-rockchip objects + stub in vmlinux ==="
ls -la "$B/arch/arm/mach-rockchip/" || true
# NOTE: no `grep -q` under pipefail — it SIGPIPEs nm/strings mid-stream and
# makes a successful match look like a failure. Plain grep + /dev/null is safe.
if nm "$B/vmlinux" | grep "__cpu_method_of_table_rk3036_smp" > /dev/null; then
  echo "SMP OK: rk3036 CPU-method table linked (platsmp.o)"
else
  echo "FATAL: mach-rockchip still not linked"; exit 1
fi
if nm "$B/vmlinux" | grep "rockchip_boot_fn"; then
  echo "TRAMPOLINE OK: rockchip_boot_fn present"
else
  echo "FATAL: headsmp trampoline missing"; exit 1
fi
if strings "$B/vmlinux" | grep "V10DIAG: blocked SIP SMC"; then
  echo "SIP STUB OK"
else
  echo "FATAL: SIP stub string missing"; exit 1
fi
if strings "$B/vmlinux" | grep "Rockchip (Device Tree)"; then
  echo "MACHINE DESC OK: mach-rockchip DT_MACHINE_START linked"
else
  echo "FATAL: machine desc missing"; exit 1
fi
