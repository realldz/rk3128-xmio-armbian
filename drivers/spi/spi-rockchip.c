...
static inline void wait_for_tx_idle(struct rockchip_spi *rs, bool target_mode)
{
	...
	if (target_mode && rs->version == ROCKCHIP_SPI_VER2_TYPE2) {
		bit_field = SR_TARGET_TX_BUSY;
		idle_val = 0;
	} else if (target_mode) {
...
static int rockchip_spi_config(struct rockchip_spi *rs,
		...
		enum rockchip_spi_xfer_mode xfer_mode, bool target_mode)
{
				      ...
				      struct spi_transfer *xfer)
{
	...
	if (spi_controller_is_target(ctlr)) {
		...
		if (rs->target_abort) {
			dev_err(rs->dev, "RK SPI transfer target abort\n");
			return -EIO;
		}
	} else {
		if (!speed_hz)
		...
		struct spi_transfer *xfer)
{
	...
	ret = rockchip_spi_config(rs, spi, xfer, xfer_mode, ctlr->target);
	if (ret)
		return ret;
...
static int rockchip_spi_setup(struct spi_device *spi)
{
	...
	if (spi->mode & SPI_CS_HIGH && spi_get_chipselect(spi, 0) <= 1 && !spi_get_csgpiod(spi, 0))
		cr0 |= BIT(spi_get_chipselect(spi, 0)) << CR0_SOI_OFFSET;
	else if (spi_get_chipselect(spi, 0) <= 1 && !spi_get_csgpiod(spi, 0))
		cr0 &= ~(BIT(spi_get_chipselect(spi, 0)) << CR0_SOI_OFFSET);
...
static int rockchip_spi_probe(struct platform_device *pdev)
{
	...
	if ((rs->poll || rs->retry_poll) && target_mode) {
