...
#define	OS12D40_EXPOSURE_MIN		8 //MAX VTS-16
#define	OS12D40_EXPOSURE_STEP		1
#define OS12D40_VTS_MAX			0xffff

#define OS12D40_REG_GAIN_H		0x3508
#define OS12D40_REG_GAIN_L		0x3509
#define OS12D40_REG_DGAIN_H		0x350A
#define OS12D40_REG_DGAIN_M		0x350B
#define OS12D40_REG_DGAIN_L		0x350C
#define OS12D40_GAIN_L_MASK		0xff
#define OS12D40_GAIN_H_MASK		0x3f
#define OS12D40_GAIN_H_SHIFT		8
#define	ANALOG_GAIN_MIN			0x80
#define	ANALOG_GAIN_MAX			0x3e000
...
#define OS12D40_REG_TEST_PATTERN	0x5081
#define	OS12D40_TEST_PATTERN_ENABLE	0x01
#define	OS12D40_TEST_PATTERN_DISABLE	0x0
...
static const struct regval os12d40_global_regs[] = {
	{REG_NULL, 0x00},
};
 ...
 * Xclk 24Mhz
 * Pclk 398.4Mhz
 * linelength 1080(0x438)12us
 * framelength 2776(0xad8)
 * grabwindow_width 4512
 * grabwindow_height 2512
 * max_framerate 30fps
 * mipi_datarate per lane 996Mbps
 */
static const struct regval os12d40_4512x2512_regs_4lane[] = {
...
static const struct os12d40_mode supported_modes_4lane[] = {
	...
	},
	{
		...
		.hts_def = 0x438 * 8,
		.vts_def = 0x0ad8,
		.reg_list = os12d40_4512x2512_regs_4lane,
		.hdr_mode = NO_HDR,
	},
};
...
static const char * const os12d40_test_pattern_menu[] = {
	"Disabled",
...
static int os12d40_enable_test_pattern(struct os12d40 *os12d40, u32 pattern)
{
	u32 val;

	if (pattern)
		val = ((pattern - 1) << 4) | OS12D40_TEST_PATTERN_ENABLE;
	else
		val = OS12D40_TEST_PATTERN_DISABLE;

	/* test pattern select*/
	return os12d40_write_reg(os12d40->client, OS12D40_REG_TEST_PATTERN,
				OS12D40_REG_VALUE_08BIT, val);
}
...
static void os12d40_set_awb_cfg(struct os12d40 *os12d40,
				 struct rkmodule_awb_cfg *cfg)
{
	mutex_lock(&os12d40->mutex);
	memcpy(&os12d40->awb_cfg, cfg, sizeof(*cfg));
	mutex_unlock(&os12d40->mutex);
}
...
static long os12d40_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_HDR_CFG:
		...
		hdr->hdr_mode = os12d40->cur_mode->hdr_mode;
		break;
	...
	case RKMODULE_AWB_CFG:
		os12d40_set_awb_cfg(os12d40, (struct rkmodule_awb_cfg *)arg);
		break;
				  ...
				  unsigned int cmd, unsigned long arg)
{
	...
	case RKMODULE_AWB_CFG:
		...
		ret = os12d40_ioctl(sd, cmd, awb_cfg);
		kfree(awb_cfg);
		break;
	case RKMODULE_SET_QUICK_STREAM:
		...
		ret = os12d40_ioctl(sd, cmd, &stream);
		break;
	case RKMODULE_GET_BAYER_MODE:
		ret = os12d40_ioctl(sd, cmd, &bayer_mode);
		if (!ret) {
...
static int __os12d40_start_stream(struct os12d40 *os12d40)
{
	...
	dev_info(&os12d40->client->dev, "%s:Check register value!\n",
				__func__);
	ret = os12d40_reg_verify(os12d40->client, os12d40_global_regs);
	if (ret)
		return ret;
...
static int os12d40_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		ret = os12d40_write_array(os12d40->client, os12d40_global_regs);
		if (ret) {
				...
				struct v4l2_mbus_config *config)
{
	...
	config->bus.mipi_csi2.num_data_lanes = OS12D40_LANES;
...
#define DST_HEIGHT 2512
static int os12d40_get_selection(struct v4l2_subdev *sd,
				 struct v4l2_subdev_state *sd_state,
...
static int os12d40_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		max = os12d40->cur_mode->height + ctrl->val - 16;
	...
	case V4L2_CID_EXPOSURE:
		ret = os12d40_write_reg(os12d40->client, OS12D40_REG_EXPOSURE,
					OS12D40_REG_VALUE_16BIT, ctrl->val);
		dev_dbg(&client->dev, "exposure %d\n", ctrl->val);
		break;
	case V4L2_CID_ANALOGUE_GAIN:
		/*
		 * again max 15.5x, step 128
		 * dgain max 16x, step 1024
		 * [1 15.5 128 0 1 128 1984],
		 * [15.5 248 1024 0 1 15872 253952]
		 */
		if (ctrl->val <= 1984) {
			again = ctrl->val;
			dgain = 1024;
			...
			again = 1984;
			dgain = ctrl->val * 128 / 1984;
			if (dgain >= 16383)
				dgain = 16383;
		}
		ret = os12d40_write_reg(os12d40->client, OS12D40_REG_GAIN_H,
					OS12D40_REG_VALUE_16BIT,
					(again << 1));
		ret |= os12d40_write_reg(os12d40->client, OS12D40_REG_DGAIN_H,
					 OS12D40_REG_VALUE_24BIT,
					 (dgain << 6));
		dev_dbg(&client->dev, "total gain %d, again %d, dgain %d\n",
			ctrl->val, again, dgain);
		break;
	case V4L2_CID_VBLANK:
		ret = os12d40_write_reg(os12d40->client, OS12D40_REG_VTS,
					OS12D40_REG_VALUE_16BIT,
					ctrl->val + os12d40->cur_mode->height);
		break;
	case V4L2_CID_TEST_PATTERN:
		ret = os12d40_enable_test_pattern(os12d40, ctrl->val);
		break;
	case V4L2_CID_HFLIP:
...
static int os12d40_initialize_controls(struct os12d40 *os12d40)
{
	...
	os12d40->exposure = v4l2_ctrl_new_std(handler, &os12d40_ctrl_ops,
				V4L2_CID_EXPOSURE, OS12D40_EXPOSURE_MIN,
				exposure_max, OS12D40_EXPOSURE_STEP,
				mode->exp_def);
	...
	os12d40->test_pattern = v4l2_ctrl_new_std_menu_items(handler,
				&os12d40_ctrl_ops, V4L2_CID_TEST_PATTERN,
				ARRAY_SIZE(os12d40_test_pattern_menu) - 1,
				0, 0, os12d40_test_pattern_menu);

	v4l2_ctrl_new_std(handler, &os12d40_ctrl_ops,
...
static void os12d40_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct os12d40 *os12d40 = to_os12d40(sd);
